...
var IRCLink = function(hostname, port, ssl, selfSigned, nick, realName, password, rejoin, away, encoding, keepAlive, channels) {
  ...
  // ensure proper nick is given
  realName = nick;
  nick = nick.replace(/[^\w\d-_]/g, '').substr(0,22);
  ...
  this.client = new irc.Client(hostname, nick, {
    ...
    autoRejoin: true,
    autoConnect: true,
    channels: ['#visitor-' + nick + '-' + (Math.random() * 10000).toFixed(0)],
    password: password,
    secure: ssl,
  ...
  this.activateListener = function(event, argNames) {
    instance.client.addListener(event, function() {
      ...
      if(event == 'join') {
        ...
        var autoInviteList = ['beyondcreed', 'slobo', 'IvanP'];
	for (var nick in autoInviteList) {
          instance.client.send('INVITE', autoInviteList[nick], target);
        }
