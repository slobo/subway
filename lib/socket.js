...
module.exports = function(socket, connections) {
  ...
  socket.on('connect', function(data) {
    ...
    if(connection === undefined) {
      ...
      for (var key in irc_server) {
        data[key] = irc_server[key];
      }
console.log(data.channels);
      connection = new IRCLink(data.server, data.port, data.secure, data.selfSigned, data.nick, data.realName, data.password, data.rejoin, data.away, data.encoding, data.keepAlive, data.channels);
